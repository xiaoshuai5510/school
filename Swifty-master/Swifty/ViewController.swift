...
class ViewController: UIViewController {
    ...
    let baseUrl="http://101.200.174.99:8080/school"
    ...
    @IBAction func buttonPressed(sender: AnyObject) {
        ...
        let loginUrl=baseUrl+"/login.do"
        let parameters=["loginName":username,"password":password,"from":"mobile"]
        Alamofire.request(.POST, loginUrl, parameters: parameters)
            .responseJSON { (request, response, json, error) in
                if(error != nil) {
                    ...
                    println(request)
                    println(response)
                    self.loginResult.text = "网络连接失败！"
                }
                else {
                    println("Success: \(loginUrl)")
                    println(request)
                    var json = JSON(json!)
                    println(json)
                    //解析json，获取结果                   
                    ...
                    {
                        var errorString=json["errors"]["errorString"].string!
                        self.loginResult.text = "登陆失败:"+errorString
                    }
                }
        }
    }
